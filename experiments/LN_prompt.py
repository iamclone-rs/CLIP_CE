...
if __name__ == '__main__':
    ...
    if ckpt_path is None:
        model = Model(class_names=class_names if float(getattr(opts, 'cls_loss_weight', 0.0)) > 0.0 else None)
    else:
        print ('resuming training from %s'%ckpt_path)
        model = Model.load_from_checkpoint(
            ckpt_path,
            class_names=class_names if float(getattr(opts, 'cls_loss_weight', 0.0)) > 0.0 else None,
        )
